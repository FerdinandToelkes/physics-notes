...
Hermiticity and spectra -- the case in all examples below.
...
\begin{equation}
e^{A}Be^{-A}=\sum_{n=0}^{\infty}\frac{1}{n!}\,\ad_A^n(B),\qquad
\ad_A(B)\equiv[A,B],\quad \ad_A^n(B)\equiv\underbrace{[A,[A,\dots[A}_{n},B]\dots]].
\end{equation}
\end{lemma*}

The series is exact (not merely asymptotic) whenever it converges, e.g.\ whenever the
nested commutators eventually reproduce a small, recognizable pattern.

\paragraph{Strategy 1 -- reduce to elementary operators.}
Conjugation is an algebra automorphism: for any $X,Y$,
\begin{equation}
e^{A}(XY)e^{-A}=\big(e^{A}Xe^{-A}\big)\big(e^{A}Ye^{-A}\big).
\end{equation}
It suffices to compute $e^{A}\,a\,e^{-A}$ and $e^{A}\,a^\dagger e^{-A}$ once; any
...
$e^{A}Be^{-A}=UBU^\dagger$; taking the dagger,
\begin{equation}
...
\subsection{Method B: the differential-equation (generator-flow) method}

Define $X(\theta)\equiv e^{\theta A}\,X\,e^{-\theta A}$, $X(0)=X$. Differentiating,
\begin{equation}
\frac{dX(\theta)}{d\theta}=[A,X(\theta)]=\ad_A\big(X(\theta)\big).
\end{equation}
This ODE is exactly equivalent to Method A -- the Hadamard series is its formal Taylor
solution around $\theta=0$ -- but is often more tractable in practice.
...
each at most linear, so $\{a,a^\dagger,\mathbb1\}$ is automatically closed under
...
$\pm1$ (hyperbolic), $M^2=\mathbb1$:
\begin{equation}
a(\xi)=a\cosh\xi+a^\dagger\sinh\xi.
\end{equation}

\paragraph{(b) Displacement.} $A=\alpha(a^\dagger-a)$: $[A,a]=-\alpha\,\mathbb 1$ is a
c-number, so enlarge the closed set to $\{a,\mathbb1\}$. The matrix is nilpotent,
giving polynomial growth:
\begin{equation}
a(\alpha)=a-\alpha\,\mathbb 1.
\end{equation}
