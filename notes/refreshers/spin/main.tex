...
\begin{equation}
\boxed{\,[\hat S_j,\hat S_k]=i\hbar\,\epsilon_{jk\ell}\hat S_\ell\,}\tag{$\circledcirc$}\label{eq:spin-comm}
\qquad(\hbar=1),
\end{equation}
with $i,j,k=x,y,z=1,2,3$ and using the Einstein summation convention.

One can define raising \& lowering operators as
\begin{equation}
...
&\overset{[\cdot,\cdot]\ \mathrm{bilin.}}{=}
\underbrace{[S_x,S_x]}_{=0}+i[S_x,S_y]-i[S_y,S_x]+\underbrace{[S_y,S_y]}_{=0}\notag\\
&=2i[S_x,S_y]
\overset{\eqref{eq:spin-comm}}{=}2i\cdot i\,\epsilon_{12\ell}S_\ell
=-2\,\epsilon_{12\ell}S_\ell
=-2S_z,
\end{align}
where the last step uses $\epsilon_{jk\ell}=0$ if two indices are equal, leaving
