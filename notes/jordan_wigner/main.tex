...
\begin{itemize}
\item \emph{Fermions and Jordan--Wigner String} $\sim$ ITensor C\texttt{++} Documentation,
\item \emph{Introduction to Many-Body Physics}, Chapter 4.1 $\sim$ Piers Coleman,
...
\begin{equation}
\ket{\vec n}=\ket{n_1,n_2,\dots,n_L}\equiv\Bigl(\prod_{i=1}^{L}(c_i^\dagger)^{n_i}\Bigr)\ket{0},
\end{equation}
where $\hat n_i\ket{\vec n}=c_i^\dagger c_i\ket{\vec n}=n_i\ket{\vec n}$ with $n_i=0,1$ and
\begin{equation}
[\hat n_i,\hat n_j]=0.\tag{$\star_1$}\label{eq:jw-nn}
\end{equation}
...
\begin{equation}
c_j\equiv\Bigl(\prod_{i=1}^{j-1}Z_i\Bigr)a_j\qquad\&\qquad
c_j^\dagger\equiv a_j^\dagger\Bigl(\prod_{i=1}^{j-1}Z_i\Bigr),\tag{$\ast$}\label{eq:jw-def}
\end{equation}
with $Z_j=(-1)^{n_j}=1-2n_j$ and $n_j=c_j^\dagger c_j$. The ordering of the $Z_i$ does not
matter since $[Z_i,Z_j]=0$ (using \eqref{eq:jw-nn}).

Inverting \eqref{eq:jw-def} using $Z_i^2=\mathds{1}$ and $[Z_i,Z_j]=0$ yields
\begin{equation}
a_j=\Bigl(\prod_{i=1}^{j-1}Z_i\Bigr)c_j\qquad\&\qquad
a_j^\dagger=c_j^\dagger\Bigl(\prod_{i=1}^{j-1}Z_i\Bigr).\tag{$\ast'$}\label{eq:jw-inv}
\end{equation}
...
using that
\begin{equation}
\{Z_i,c_j^{(+)}\}=0\tag{$\star_2$}\label{eq:jw-Zc}
\end{equation}
(proofs further below).
...
&\equiv\Bigl(\prod_{i=1}^{L}(c_i^\dagger)^{n_i}\Bigr)\ket{0}
&&\text{w.\ }n_i=0,1\notag\\
&=\Bigl\{\prod_{i=1}^{L}\Bigl[a_i^\dagger\Bigl(\prod_{j=1}^{i-1}Z_j\Bigr)\Bigr]^{n_i}\Bigr\}\ket{0}
&&(c_1^\dagger=a_1^\dagger)\notag\\
&=(a_1^\dagger)^{n_1}(a_2^\dagger Z_1)^{n_2}(a_3^\dagger Z_2 Z_1)^{n_3}\cdots
  (a_L^\dagger Z_{L-1}\cdots Z_1)^{n_L}\ket{0}\notag\\
&=\Bigl(\prod_{i=1}^{L}(a_i^\dagger)^{n_i}\Bigr)
  (Z_1)^{n_2}(Z_2 Z_1)^{n_3}\cdots(Z_{L-1}\cdots Z_1)^{n_L}\ket{0}\notag\\
&=\Bigl(\prod_{i=1}^{L}(a_i^\dagger)^{n_i}\Bigr)\ket{0}
&&\text{w.\ }n_i=0,1,
\end{align}
...
&=a_\ell\Bigl(\prod_{i=1}^{L}(a_i^\dagger)^{n_i}\Bigr)\ket{0}\notag\\
&\overset{[a_\ell,a_j^\dagger]=0}{=}\Bigl(\prod_{i=1}^{L}(a_i^\dagger)^{n_i}\Bigr)a_\ell\ket{0}=0,
\end{align}
since $a_\ell\ket{0}=c_\ell\ket{0}=0$. For $n_\ell=1$,
\begin{align}
a_\ell\ket{n_1,\dots,n_\ell=1,\dots,n_L}
&=a_\ell\Bigl(\prod_{\substack{i=1\\i\neq\ell}}^{L}(a_i^\dagger)^{n_i}\Bigr)a_\ell^\dagger\ket{0}\notag\\
&\overset{[a_\ell,a_j^\dagger]=0}{=}
\Bigl(\prod_{\substack{i=1\\i\neq\ell}}^{L}(a_i^\dagger)^{n_i}\Bigr)a_\ell a_\ell^\dagger\ket{0}
=\Bigl(\prod_{\substack{i=1\\i\neq\ell}}^{L}(a_i^\dagger)^{n_i}\Bigr)c_\ell c_\ell^\dagger\ket{0}\notag\\
&=\Bigl(\prod_{\substack{i=1\\i\neq\ell}}^{L}(a_i^\dagger)^{n_i}\Bigr)\ket{0}
...
\S\ref{sec:spin-refresher}). For instance
\begin{equation*}
...
\subsection{Proofs about the JWT}
...
Z_j c_j\ket{1}_j&=Z_j\ket{0}_j=\ket{0}_j=c_j\ket{0}_j,\\
...
\begin{proof}
Assume w.l.o.g.\ that $i<j$. Using \eqref{eq:jw-inv},
\begin{align}
a_j a_i&=\bigl(Z_1\cdots Z_{j-1}c_j\bigr)\bigl(Z_1\cdots Z_{i-1}c_i\bigr)\notag\\
...
and likewise
\begin{align}
a_i a_j&=\bigl(Z_1\cdots Z_{i-1}c_i\bigr)\bigl(Z_1\cdots Z_{j-1}c_j\bigr)\notag\\
...
\begin{proof}
We can show it w.l.o.g.\ for $i<j$. Using \eqref{eq:jw-inv} and \eqref{eq:jw-Zc},
\begin{align}
...
\overset{[Z_j,c_\ell^{(+)}]=0,\,j\neq\ell}{=}
Z_i\cdots Z_{j-1}\,c_j c_i^\dagger\,\underbrace{Z_{i-1}^2\cdots Z_1^2}_{=\mathds 1}
=Z_i\cdots Z_{j-1}\,c_j c_i^\dagger .
\end{align}
Both reduce to the same expression, so $a_j a_i^\dagger-a_i^\dagger a_j=0$ for $i<j$, i.e.\
$[a_j,a_i^\dagger]=0$; the case $i>j$ follows straightforwardly.\hfill(ii)
\end{proof}

\paragraph{Claim: $[a_i,a_i^\dagger]=1-2n_i$ with $n_i=c_i^\dagger c_i$.}
\begin{proof}
Using \eqref{eq:jw-inv} and $Z_k^2=\mathds 1$,
\begin{align}
